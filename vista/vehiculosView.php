...
<main>
    ...
    <main>
        <?php if (isset($vehiculo)) { ?>
            <table>
                ...
                foreach ($vehiculo as $row) {
                    ...
                    ?>
                    <form action = "<?php echo $this->url("web", "borrarModiVehiculo"); ?>" method = "post">
                        <input name = "matricula" type = "text" hidden value = "<?php echo $row['matricula']; ?>">
        ...
        <hr>
        <form action="<?php echo $this->url('web', 'buscarBastidor'); ?>" method="POST">
            <label for="bastidor">Buscar por bastidor</label>
            <input type="text" name="bastidor" id="bastidor">
            <input type="submit" value="Enviar">
        </form>
        <form action="<?php echo $this->url('web', 'buscarMatricula'); ?>" method="POST">
            <label for="matricula">Buscar por matricula</label>
            <input type="text" name="matricula" id="matricula">
            <input type="submit" value="Enviar">
        </form>
        <form action="<?php echo $this->url('web', 'buscarDNI'); ?>" method="POST">
        ...
        <form action="<?php echo $this->url('web', $modo); ?>" method="POST">
            ...
            if (isset($vehiculoModi)) {
                echo $vehiculoModi[0]['matricula'];
            }
            ?>">
            <label for="bastidor">Bastidor</label>
            <input type="text" name="bastidor" id="bastidor" value="<?php
            if (isset($vehiculoModi)) {
            ...
            <input type="number" name="telefono" id="telefono" value="<?php
                   if (isset($vehiculoModi)) {
